...
class ImageCubePILobject:
    def __init__(self, folio_dir, folio_name, band_list, rotate_angle):
        ...
        self.band_list = band_list #read_band_list(os.path.join(self.folio_dir,"band_list.txt"), modalities)
    ...
    def get_image_size(self):
        ...
        :return:
        """
        pil_msi_obj = self.read_pil_band_ims_list()
        width = pil_msi_obj[0].size[0]
        height = pil_msi_obj[0].size[1]
        for pil_obj in pil_msi_obj:
           pil_obj.close()
        return width,height

    def read_image_object(self,path, rotate_angle):
        im = Image.open(path)
        if rotate_angle > 0:
            rotation = eval("Image.ROTATE_{}".format(rotate_angle))
            im = im.transpose(rotation)
        return im

    def read_pil_band_ims_list(self):
        ...
        for idx, band_name in enumerate(self.band_list):
            ...
            # in case name separator is + not -
            if not os.path.exists(fpath):
                fpath = os.path.join(self.image_dir, self.folio_name + "+" + band_name + ".tif")
            im = self.read_image_object(fpath, self.rotate_angle)
            msi_img.append(im)
        return msi_img
...
class ThumbnailMSI_PIL(ImageCubePILobject):
    def __init__(self,image_dir,folio_name,band_list,rotate_angle,scale_ratio):
        """Resize msi_img_obj"""
        super().__init__(image_dir, folio_name, rotate_angle, band_list)
        self.width = self.width // scale_ratio
        self.height = self.height // scale_ratio
        self.pil_msi_img = self.read_pil_band_ims_list()
