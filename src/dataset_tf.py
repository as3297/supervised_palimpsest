...
def create_dataset_coords(coords, labels,main_paths, palimpsest_names,shuffle=True):
    ...
    dataset = tf.data.Dataset.from_tensor_slices(
        {"coords": tf_coords, "labels": tf_labels, "main_paths": tf_main_paths, "palimpsest_names": tf_palimpsest_names,
...
def create_oversampled_dataset(batch_size,pal_names,main_dir,modality,shuffle,filter_label_noise, test_on_subset):
    ...
    for pal_name in pal_names:
        ...
        if test_on_subset:
            coord_ut, coord_nonut = coord_ut[:min(32,nb_ut_samples)], coord_nonut[:min(32,nb_nonut_samples)]
            nb_ut_samples = len(coord_ut)
            nb_nonut_samples = len(coord_nonut)
    ...
    coords_ut = np.concatenate(coords_ut,axis=1)
    coords_nonut = np.concatenate(coords_nonut,axis=1)
    ...
    resampled_ds = resampled_ds.prefetch(buffer_size=tf.data.AUTOTUNE)
    if test_on_subset:
        resampled_ds = resampled_ds.map(processing_test,num_parallel_calls=tf.data.AUTOTUNE)
    else:
        resampled_ds = resampled_ds.map(processing, num_parallel_calls=tf.data.AUTOTUNE)
    return resampled_ds
...
def create_oversampled_dataset_test():
    ...
    for element in resample_ds.take(1):
        print(element)
